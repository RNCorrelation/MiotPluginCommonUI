...
import {
  Dimensions, Modal, Platform, StyleSheet, Text, TouchableHighlight, TouchableWithoutFeedback, View
} from 'react-native';
...
/**
 * copy 小米，修正subtitle,current 不能动态更新。
 *
 *
 * add in componentWillReceiveProps() =>

    if (newProps.current !== this.state.current) {
      this.setState({
        currentArray: newProps.current,
        subtitle: this.getSubtitle(newProps.current)
      });
    }
 *
 */
export default class LHDatePicker extends React.Component {
  static propTypes = {
  ...
  initByNewProps(newProps) {
    if (!newProps) {
      return;
    }
    const { type, singleType, current, min, max } = newProps;
    const currentArray = this.convert(current || new Date());
    switch (type) {
      case TYPE.DATE:
        const yearNow = new Date().getFullYear();
  ...
  componentWillReceiveProps(newProps) {
    ...
    if (newProps.current !== this.state.currentArray) {
      const { currentArray, dataSourceArray } = this.initByNewProps(newProps);
      this.setState({
        currentArray,
        dataSourceArray,
    ...
    return (
      <View style={styles.pickerContainer}>
        {dataSourceArray.map((dataSource, index) => {
          ...
          } else if (this.props.type === TYPE.SINGLE) {
            if (this.props.replaceZero) {
              if (!dataSource.includes(this.props.replaceZero)) {
                dataSource.unshift(this.props.replaceZero);
              }
            }
          }
          return (
            <View style={[{ flexDirection: 'row' }, style]}>
              <StringSpinner
                ...
                style={style}
                unit={this.props.showUnit ? this.unitArray[index] : ''}
